...
author: "Kerry Du and Huanyu Jia"
affiliation: "School of Management, Xiamen University; School of Business, Zhengzhou University"
contact: "kerrydu@xmu.edu.cn; jiahuanyu@zzu.edu.cn"
date: "March 2026"
---
...
This paper introduces `ishere` and `tohtml`, two Stata commands designed to automate the conversion of Stata do-files and log files into clean Markdown and HTML reports. Unlike existing comprehensive solutions that offer extensive features and polished output, this package deliberately prioritizes simplicity and ease of adoption. The design philosophy, captured in the phrase "everything is here: from do to html," emphasizes minimalist implementation with the lowest possible learning curve. Users mark key locations in their do-files with simple `ishere` commands, run their analyses as usual, and use `tohtml` to transform the log into clean Markdown and html—all without leaving the Stata environment or adopting new programming paradigms. Should the auto-generated HTML fail to meet specific use case requirements, the original Markdown output serves as a production-ready draft containing all Stata-derived core elements: code snippets, analytical results, figures, and tables. This Markdown file can then be further refined with powerful dedicated editors such as Typora, VS Code, or Obsidian. This two-stage workflow (Stata-to-Markdown, then Markdown-to-final-document) offers flexibility and leverages the strengths of specialized editing tools while maintaining simplicity in the Stata component.
...
5. **Directory Processing**: Can process entire directories of figures and tables into markdown/HTML files automatically
...
```stata
* Start logging
log using "example1.log", text replace
...
* Convert to HTML
tohtml example1.md, html(example1.html) css(githubstyle) replace
```
...
log close
tohtml example2.log, html(example2.html) css(githubstyle) replace
...
ishere tab using "regression_table.html",height(700px) width(900px)
...
ishere
sysuse auto, clear
logoute, save("mytable") replace html : tabstat price mpg weight, s(mean sd min max)
...
ishere
regress price mpg weight
outreg2e using "main_results", replace html
ishere tab using "main_results.html"

log close
tohtml example5.log, clean html(example5.html) css(githubstyle) replace
```

The `clean` option removes all console output and code blocks, keeping only headings, narrative text (from `ishere /* ... ishere */` blocks), figures, and tables. This mode is ideal for client reports or presentations where the focus is on results rather than technical details.
...
```stata
cap log close
log using "example6.log", replace
...
log close
tohtml example6.log, cleancode(example6.do) html(example6.html) css(githubstyle) replace
```
...
- **outreg2e/esttab**: Export regression tables to HTML
- **table/logoute**: Generate formatted tables
...
```stata
net install stata_tohtml, ///
    from("https://github.com/kerrydu/Stata_tohtml")
```
