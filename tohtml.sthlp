...
{pmore}
Due to a current limitation of Stata's official {cmd:markdown} command,
image paths must not contain spaces when {opt embed} is specified.
This includes spaces in folder names and image filenames.
We have contacted StataCorp about this issue and are awaiting a fix.
