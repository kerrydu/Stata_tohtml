*! outreg2e extend outreg2 for creating tables in html
* most code is from original outreg2.ado
prog define outreg2e, by(onecall) sortpreserve
	versionSet
	version `version'
...
syntax [anything] [using] [if] [in] [pweight fweight aweight iweight] [, drop(str) keep(str) * IShere ISHEREtext(string)]
if "`drop'"~="" & "`keep'"~="" {
...
syntax [anything] [using] [if] [in] [pweight fweight aweight iweight] [, SEEout REPLACE NOSEEOUT NOREPLACE CROSStab AGAIN * CDOUT  IShere ISHEREtext(string)]
	...
	* regular stuff
	if `"`using'"'~="" {
		...
		* prefix shorthand synatx
		syntax [anything] [, REPLACE SEEout APpend cdout IShere ISHEREtext(string) ]
...
syntax [anything] [using] [if] [in] [pweight fweight aweight iweight] [, REPLACE NOREPLACE SUM SUM2(str) CROSStab SEEout /*
	*/ NOSEEOUT CTTOP CTBOT CDOUT  IShere ISHEREtext(string) * ]
...
* 输出 html iframe（放在文件创建信息之后）
if `"`isheretext'"'!="" local ishere ishere 
if `"`cl_iframe_line'"'~="" & "`ishere'"!="" {
	noi di as txt "`cl_iframe_open'"
	noi di as txt `"`cl_iframe_line'"'
	// noi di as txt "`cl_iframe_close'"
	display
	if `"`isheretext'"'~="" {
		noi di as txt `"`isheretext'"'
	}
}
...
*/ POLicy0(str asis) skip NOOMITted	NOBAse noDEPVARshow IShere ISHEREtext(string)]
		...
		// di "hhh:`texFile' `titleWide' `headBorder' `bottomBorder'"
			if `hastex' {
				* make certain `1' is not `using' (another context)
				...
				if `"`texFile'"'=="" {
					local texEndName "tex"
				}
				else {
					local texEndName "`texFile'"
				}
			
				local usingTerm `"`strippedname'.`texEndName'"'
			...
			if `html' {
				...
				_texout2html v* using `"`strippedname'"', titleWide(`titleWide') headBorder(`headBorder') bottomBorder(`bottomBorder') `texopts' replace
				local htmlEndName "html"
				local usingTermHtml `"`strippedname'.`htmlEndName'"'
				c_local cl_html `"{stata `"shellout using `"`usingTermHtml'"'"':`usingTermHtml'}"'
...
syntax using/, titlefile(str) [sideway onecol sortcol(str) sortvar(str) groupvar(str) /*
	*/  noQUOte comma slow(numlist) IShere ISHEREtext(string)]
...
syntax , b(str) vc(str) [se margucp(str) IShere ISHEREtext(str)]
...
*/ stnum(str asis) ststr(str asis) eretrun wide NOOMITTED NOBASE DEPVARshow(str) IShere ISHEREtext(str)]
...
prog define fmtgen
	...
	syntax [varlist(default=none)] [in], replace(str asis) fmt(str asis) [dec(int 3) auto(int 3) less(int 0) /*
		*/ width(int 12) gen(str asis) decmark(str asis) IShere ISHEREtext(str)]
...
syntax, input(str) [dec(integer 3) less(integer 0) parse(str) strict IShere ISHEREtext(str)]
...
* 2013 04 set version 8 and moved versionSet to bottom
	...
	* add one if only one v* column exists
	...
	syntax varlist using/, titleWide(int) headBorder(int) bottomBorder(int)			/*
		...
		*/	Appendpage noPAgenum a4 a5 b5 LETter LEGal EXecutive replace IShere ISHEREtext(str)]
	if `totrows'==0 {
		local totrows = _N
	}
	...
	*/
	...
	quietly {
		...
		* make sure there are 2+ "$" in varname
		...
		gen str20 `padN' = ""
		if `titleWide' {
			replace `pad0' = "{\centering `blarge'" in 1 / `titleWide'
			replace `padN' = "`elarge' \par}" in 1 / `titleWide'
		}
		...
		* OLD logic: gen long `order' = _n + `texheadrow' in 1 / `totrows'
		gen long `order' = .
		if `totrows' > `titleWide' {
			replace `order' = _n + `texheadrow' in `=`titleWide'+1' / `totrows'
		}
		if `titleWide' > 0 {
			replace `order' = `texheadrow' - 1 + _n in 1 / `titleWide'
		}
		
		local newtexhrow1 = `totrows' + 1
		local newtexhrowN = `totrows' + `texheadrow'
		
		* OLD logic: replace `order' = _n - `totrows' in `newtexhrow1' / `newtexhrowN'
		replace `order' = _n - `totrows' in `newtexhrow1' / `newtexhrowN'
		if `titleWide' > 0 {
			replace `order' = `order' + `titleWide' if `order' == `texheadrow'
		}
...
prog define _texout2html, sortpreserve
	* HTML exporter with _texout-compatible syntax
	version 8
	...
	* align with _texout: move EQUATION labels into stub column
	...
	if !_rc & !`repflag' {
		di as err "_texout2html: file `outfile' exists; use replace"
		exit 602
	}
	...
	quietly {
		...
		file open htmlout using `"`outfile'"', write text `repopt'
		if !`fragment' {
			file write htmlout "<!DOCTYPE html>" _n
		...
		* title here, before table
		...
		if `headBorder'>`titleWide' {
			file write htmlout "<thead>" _n
			forvalues r = `=`titleWide'+1'/`headBorder' {
				local headClass ""
				if `needline' & `r'==`headBorder' {
					local headClass " class='texout-headline'"
				}
				if `r' <= `titleWide' {
					local text = `varname'[`r']
					local text = subinstr("`text'","&","&amp;",.)
					...
					foreach col of local varlist {
						...
						file write htmlout "<th>`cell'</th>" _n
					}
					file write htmlout "</tr>" _n
				}
			}
			file write htmlout "</thead>" _n
		}
		...
		if `bodyStart' <= `totrows' {
			...
			forvalues r = `bodyStart'/`totrows' {
				if `r'>_N continue
				local rowClass ""
				if `needline' & `r'==`bottomBorder' {
					local rowClass " class='texout-bottomline'"
				}
				...
				if `noteRow' {
					...
					foreach col of local varlist {
						...
						if trim("`cell'")=="" local cell = "&nbsp;"
						if "`col'"=="`varname'" {
							file write htmlout "<td>`cell'</td>" _n
						}
						else {
							file write htmlout "<td class='texout-mono'>`cell'</td>" _n
						}
					}
					file write htmlout "</tr>" _n
				}
...
syntax using/ [, excelFile(str) LEGible noNAMes titleWide(integer 0) /*
	*/ headBorder(integer 10) bottomBorder(integer 10) outreg2 labeloption(str) insert excel1(str) IShere ISHEREtext(str)]
...
syntax, saving(str) dsVarlist(str) num(numlist) n(numlist) style(str) style1(str) defaultcellstyle(str) [IShere ISHEREtext(str)]
...
prog define dsCol
	...
	* alternatively use -qui describe, short- and r(k)
...
syntax varlist 							/*
	...
	*/ DISplay log regress IShere ISHEREtext(str)]
...
prog define optionSyntax
	...
	* nameShow: mesaage to user when invalid
	...
	syntax, valid(str) name(str) nameShow(str) [content(str) PASSthru NORETURN IShere ISHEREtext(str)]
...
syntax [varlist(ts `fv')] [using] [if] [in] [pweight fweight aweight iweight] [,	/*
	*/ APpend REPLACE esample drop(str) 				/*
	*/ noDISplay log REGress DETail NODEPendent raw IShere ISHEREtext(str)]
...
syntax [anything], names(str) [number(str) IShere ISHEREtext(str)]
...
syntax [anything], names(str) [number(str asis) subtract(str) IShere ISHEREtext(str)]
	local collect ""
...
syntax using [, noQUOte comma title(str) label labelOption(str) /*
	*/ titlefile(str) NOTITLE slow(numlist) label_file(str) IShere ISHEREtext(str)]
...
syntax using/, [save(str) begin(numlist max=1) end(str) clear parse(str) replace semiclear IShere ISHEREtext(str)]
...
syntax, [cont NOOPEN IShere ISHEREtext(str)]
...
prog define _texout_parse
	version 7
	syntax, [FRagment NOPRetty PRetty Landscape IShere ISHEREtext(str)]
...
prog define _xposea_parse
	version 7
	syntax, [WHole IShere ISHEREtext(str)]
	c_local whole `whole'
end
...
prog define _excel_parse
	version 7
	syntax, [font(str) FONTSize(int 10) COLWidth(numlist) NUMeric IShere ISHEREtext(str)]
...
prog define _stats_parse
	...
	syntax, [cmd(str asis) str(str asis) r(str asis) Mat(str asis) e(str asis) IShere ISHEREtext(str)]
...
prog define eretSet, eclass
	...
	syntax varlist,[IShere ISHEREtext(str)]
	cap reg `varlist'
	if _rc==0 {
		marksample touse
		tempname ebmat eVmat
...
prog define givetoken
	* parse will take only one character
	version 7
	syntax, input(str) clocal(str) [parse(str) IShere ISHEREtext(str)]
	local num 1
	if `"`parse'"'=="" {
		local parse " "
...
prog define fvtsunab
	syntax [anything], [onebyone poundsign IShere ISHEREtext(str)]
...
prog _addstat_parse
	cap version 12
	syntax, addstat(str asis) [ADec(numlist int >=0 <=11 max=1) AFmt(str asis) AUTO(integer 3) LESS(integer 0) NOAUTO	DECMark(str asis) IShere ISHEREtext(str)]
