...
{pstd}
To include an HTML table in a report, export the table with
{cmd:outreg2e} and then use {cmd:ishere tab using} to reference
the generated HTML file while the log is open.
After closing the log, use {cmd:tohtml} to generate the report.
...
{pstd}
{bf:3. Include a Regression Table in a Report}

{pstd}
With the commands and their dependencies installed, run the following
in a writable working directory with no unnamed log open:

{phang2}{cmd:. log using "report.log", text replace}{p_end}
{phang2}{cmd:. sysuse auto, clear}{p_end}
{phang2}{cmd:. regress price mpg weight}{p_end}
{phang2}{cmd:. outreg2e using "regtable", replace html}{p_end}
{phang2}{cmd:. ishere tab using "regtable.html"}{p_end}
{phang2}{cmd:. log close}{p_end}
{phang2}{cmd:. tohtml "report.log", replace}{p_end}
