*! logout3: thin wrapper that reuses logout + dta output to build HTML tables
program define logout3
    version 17.0
    
    local original `"`0'"'

    // --- Parsing Logic adapted from logout.ado ---
    // Split 0 into options (left) and command (right) using colon as delimiter
    gettoken first second : 0, parse(":") match(par) quotes
    local 0 `"`first'"'
    
    // Scan for the colon
    while `"`first'"'~=":" & `"`first'"'~="" {
        gettoken first second : second, parse(":") match(par) quotes
    }
    
    // Check if we found a colon or just reached the end
    local colonFound = ("`first'" == ":")
    
    // Handle the shorthand syntax where 0 might just be ":"
    if `"`0'"'==":" {
        local 0
    }

    // Now `0` contains the options part. `second` contains the command part.
    ...
    // 2. Ensure dta option is present
    if "`dta'" == "" {
        local dta "dta"
    }

    // 3. Reconstruct options for the underlying logout call
    // We must pass all options except 'html'.
    // We already parsed them into macros.
    local opts ""
    if `"`use'"' != ""    local opts `"`opts' use(`"`use'"')"'
    if `"`save'"' != ""   local opts `"`opts' save(`"`save'"')"'
    if `"`fix1'"' != ""   local opts `"`opts' fix1(`fix1')"'
    if "`clear'" != ""    local opts `"`opts' clear"'
    if "`fix'" != ""      local opts `"`opts' fix"'
    if `"`fixcut'"' != "" local opts `"`opts' fixcut(`fixcut')"'
    if "`text'" != ""     local opts `"`opts' text"'
    if "`smcl'" != ""     local opts `"`opts' smcl"'
    if `"`range'"' != ""  local opts `"`opts' range(`range')"'
    if "`dta'" != ""      local opts `"`opts' dta"'
    if "`replace'" != ""  local opts `"`opts' replace"'
    if `"`options'"' != "" local opts `"`opts' `options'"'

    // 4. Construct the command to run
    // If we found a colon, append " : " + second.
    if `colonFound' {
        local cmdToRun `"logout2, `opts' : `second'"'
    }
    else {
        local cmdToRun `"logout2, `opts'"'
    }

    // 5. Execute logout (creates the .dta file)
    capture noisily `cmdToRun'
    if _rc {
        exit _rc
    }
...
program define _logout3_write_html
    version 17.0
	...
	if `"`use'"'~="" {
		...
		* `second' could contain " " only
		...
		if `"`a'"'~="" {
			...
			noi caplog2 using `"`capfile'"', tempfile replace `text' `smcl' subspace: `second'
...
if `"`filename'"'~="" {
       capture log using `"`filename'"', append
}

end

program define _caplog2
version 7
