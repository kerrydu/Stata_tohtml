...
void function rewrite_md(string scalar ofi, string scalar tfi, real scalar replace, string scalar rpath, string scalar llp)
{
    ...
    flag2 = (rem:== "") + (rem:== "0") 
    flag2 =flag2 + (rem:== "1")+ (rem:== "begin") 
    flag2 =flag2 + (rem:== "end") + (rem:== ("```"))
    flag = flag:* flag2
    ...
    fcon = remove_dotishere(fcon)
    // fcon = move_img_iframe(fcon)
    // 将 . textcell */ 或 . textcell */  换为textcell */ 和 textcell */
...
void function merge_cmdlog_blocks(string scalar clean_md, string scalar cmdlog_md, string scalar out_md, real scalar replace, string scalar rpath, string scalar llp)
{
    ...
    while (i <= n_cmd) {
        ...
        if (ustrpos(line_trim, "ishere") == 1) {
            ...
            if (rem == "" | rem == "0" | rem == "1" | rem == "begin" | rem == "end"| rem == "```") {
                result = result \ "```"
                i = i + 1
                continue
            }
...
string colvector clean_textcell_content(string colvector lines)
{
  ...
  r1 = ustrpos(lines2, "_textcell") :== 1
  
  lines2 = strltrim(substr(lines2,ustrlen("_textcell")+1,.))
  ...
  if (sum(r12)!=sum(r22)){
       errprintf("Error: unmatched _textcell /* and */\n")
       _error(199)
   }
  if (sum(r12)==0){ 
    return(lines)
   ...
   if (length(idx12)>0){
       for (i=1;i<=length(idx12);i++){
           ...
           if ((i+1) < length(idx12)) {
                ...
                errprintf("Error: overlapping _textcell /* and */\n")
                _error(199)    
               }
...
string colvector remove_dotishere(string colvector lines)
{
   r1 = (ustrpos(lines, ".") :== 1)
   lines2 = strtrim(substr(lines, 2, .))
   r2 = (ustrpos(lines2, "ishere") :== 1)
   lines3 = substr(lines2, strpos(lines2, "ishere"):+ strlen("ishere")+1, .)
   lines3 = ustrtrim(lines3)
   r3 = (ustrpos(lines3, "#") :== 1)
//    r4 = (ustrpos(strlower(lines3), "tab") :== 1)
//    r5 = (ustrpos(strlower(lines3), "fig") :== 1)
//    flag = r1 :& r2 :& (r3 | r4 | r5)
    flag = r1 :& r2 :& r3
    if (sum(flag) > 0) {
        idx = selectindex(flag)
        lines[idx] = J(rows(idx), 1, "")
    }
...
string colvector move_img_iframe(string colvector lines)
{
    flags1 = (ustrpos(lines, "<img") :== 1) :| (ustrpos(lines, "<iframe") :== 1)
    flag2 = ustrpos(lines,".") :== 1
    lines2 =strtrim(substr(lines,2,.))
    flag3 = ustrpos(lines2,"ishere") :== 1
    lines3 = substr(lines2, strpos(lines2, "ishere"):+ strlen("ishere")+1, .)
    lines3 = ustrtrim(lines3)
    flag4 = (ustrpos(strlower(lines3), "tab") :== 1) :| (ustrpos(strlower(lines3), "fig") :== 1)
    flag5 = flag2:*(flag3 +flag4)
    if (sum(flag5) > 0) {
        idx = selectindex(flag5)
        lines[idx] = J(rows(idx), 1, "")
    }
    // idx = selectindex(flags1)
    // if (sum(flag) > 0) {
    //     for (k=1; k<=rows(idx); k++) {
    //         i = idx[k]
    //         if (i< rows(lines) & flag5[i+1]==1){
    //             temp = lines[i]
    //             lines[i] = lines[i+1]
    //             lines[i+1] = temp
    //         }
...
string colvector function remove_dot_textcell(string colvector lines)
{
    flag =ustrpos(lines, ".") :== 1
    lines2 = strtrim(substr(lines, 2, .))
    flag2 = (ustrpos(lines2, "textcell") :== 1)
    flag = flag :& flag2
    if (sum(flag) > 0) {
        idx = selectindex(flag)
        lines[idx] = substr(lines[idx], 2,.) 
    }
    return(lines)
}
