...
program define tohtml
    syntax anything ,  [ cleanmd(string) REPlace HTML(string) CSS(string) RPath(string) WPath(string) CLEAN CLEANCODE(string)]
...
string colvector remove_dot_from_textcell(string colvector lines)
{
    ...
    ri2 = (usubinstr(lines2, " ", "", .) :== "ishere/*")
    ri3 = (usubinstr(lines2, " ", "", .) :== "ishere*/")
    flag = ri1 :& (ri2 :| ri3)
    if (sum(flag) > 0) {
        idx = selectindex(flag)
        lines[idx] = usubinstr((substr(lines[idx], 2, .)),"","",.)
...
{
   text_start = (lines:=="_ishere_/*")
   text_end = (lines:=="_ishere_*/")
