...
{pstd}
Run the following commands in a do-file from a writable working directory.
The example uses {cmd:logoute}, {cmd:ishere}, and {cmd:tohtml} from this package.
{cmd:tohtml} also requires {cmd:moremata}; if it is not installed, install it
explicitly with {cmd:ssc install moremata} before running the example.

{phang2}{cmd:. capture log close}{p_end}
{phang2}{cmd:. set linesize 80}{p_end}
{phang2}{cmd:. log using "report.log", replace text}{p_end}
{phang2}{cmd:. sysuse auto, clear}{p_end}
...
{phang2}{cmd:. tohtml "report.log", md("final_report.md") ///}{p_end}
{phang2}{cmd:      html("final_report.html") replace}{p_end}

{pstd}
{cmd:report.log} is the plain-text input log and is closed before conversion.
{cmd:ishere tab using} writes a table marker to that log; {cmd:tohtml} reads
the marker and inserts the HTML table into the report. The {opt md()} and
{opt html()} options name the generated Markdown and HTML files, respectively.
This example creates {cmd:descriptives.html}, {cmd:final_report.md}, and
{cmd:final_report.html} in the working directory. The report stylesheet is
copied to its {cmd:css} subdirectory.
